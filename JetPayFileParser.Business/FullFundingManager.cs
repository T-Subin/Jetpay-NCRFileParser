...
{
    public class FullFundingManager
    {
        ...
        public bool BulkInsertAuth(int clientId, string filePath, StreamWriter objTextWriter, string InputPath, string MoveFilePathWithName, string FileMovePath)
        {
            ...
            try
            {
                ...
                fileImportLogId = fileImportLogManager.InsertDataToFileImportLog(fileName, FileType.FundingFull, fileInfo.LastWriteTime);
                
                DataTable dtSchema = commonDAL.GetTableSchema(AppConstants.Table_FundingFull);
                DataTable dtColumnMappings = commonDAL.GetColumnMappingsByFileTypeId((int)FileType.FundingFull);

                FetchDataInfo objFetchDataInfo = BulkDataInsertManager.FetchDataFromCSV(MoveFilePathWithName, AppConstants.Table_FundingFull,
                ...
                if (dt.Rows.Count > 0)
                {
                    try
                    {
                        ...
                        BulkDataInsertManager.BulkInsertWithColumnMapping(AppConstants.Table_FundingFull, dt, GetColumnMappings(), false);
