...
{
    public class FundingCategoryManager
    {
        FundingCategoryDAL fundingCategoryDAL = new FundingCategoryDAL();
        ...
        public bool BulkInsertAuth(int clientId, string filePath, StreamWriter objTextWriter, string InputPath, string MoveFilePathWithName, string FileMovePath)
        {
            ...
            try
            {
                ...
                fileImportLogId = fileImportLogManager.InsertDataToFileImportLog(fileName, FileType.FundingCategory, fileInfo.LastWriteTime);

                DataTable dtSchema = commonDAL.GetTableSchema(AppConstants.Table_FundingCategory);
                DataTable dtColumnMappings = commonDAL.GetColumnMappingsByFileTypeId((int)FileType.FundingCategory);

                FetchDataInfo objFetchDataInfo = BulkDataInsertManager.FetchDataFromCSV(MoveFilePathWithName, AppConstants.Table_FundingCategory,
                ...
                if (dt.Rows.Count > 0)
                {
                    try
                    {
                        ...
                        BulkDataInsertManager.BulkInsertWithColumnMapping(AppConstants.Table_FundingCategory, dt, GetColumnMappings(), false);
                        ...
                        fundingCategoryDAL.MoveDataToFundingCategory(fileImportLogId);
