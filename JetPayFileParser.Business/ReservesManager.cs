    ...
    public class ReservesManager
    {
        ...
        public bool BulkInsertAuth(int clientId, string filePath, StreamWriter objTextWriter, string InputPath, string MoveFilePathWithName, string FileMovePath)
        {
            ...
            try
            {
                ...
                fileImportLogId = fileImportLogManager.InsertDataToFileImportLog(fileName, FileType.Reserves, fileInfo.LastWriteTime);

                DataTable dtSchema = commonDAL.GetTableSchema(AppConstants.Table_Reserves);
                DataTable dtColumnMappings = commonDAL.GetColumnMappingsByFileTypeId((int)FileType.Reserves);

                objTextWriter.WriteLine("Fetching Data From CSV....: {0}----------", DateTime.Now);
                FetchDataInfo objFetchDataInfo = BulkDataInsertManager.FetchDataFromCSV(MoveFilePathWithName, AppConstants.Table_Reserves,
                ...
                if (dt.Rows.Count > 0)
                {
                    try
                    {
                        ...
                        BulkDataInsertManager.BulkInsertWithColumnMapping(AppConstants.Table_Reserves, dt, GetColumnMappings(), false);
                        ...
                        reservesDAL.MoveDataToReserves(fileImportLogId);
