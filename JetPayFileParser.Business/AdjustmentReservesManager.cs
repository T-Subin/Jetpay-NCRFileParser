...
{
    public class AdjustmentReservesManager
    {
        ...
        public bool BulkInsertAuth(int clientId, string filePath, StreamWriter objTextWriter, string InputPath, string MoveFilePathWithName, string FileMovePath)
        {
            ...
            try
            {
                ...
                fileImportLogId = fileImportLogManager.InsertDataToFileImportLog(fileName, FileType.AdjustmentReserves, fileInfo.LastWriteTime);
                
                DataTable dtSchema = commonDAL.GetTableSchema(AppConstants.Table_AdjustmentReserves);
                DataTable dtColumnMappings = commonDAL.GetColumnMappingsByFileTypeId((int)FileType.AdjustmentReserves);

                FetchDataInfo objFetchDataInfo = BulkDataInsertManager.FetchDataFromCSV(MoveFilePathWithName, AppConstants.Table_AdjustmentReserves,
                ...
                if (dt.Rows.Count > 0)
                {
                    try
                    {
                        ...
                        BulkDataInsertManager.BulkInsertWithColumnMapping(AppConstants.Table_AdjustmentReserves, dt, GetColumnMappings(), false);
                        ...
                        adjustmentReservesDAL.MoveDataToAdjustmentReserves(fileImportLogId);                        
