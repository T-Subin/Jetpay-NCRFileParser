...
namespace JetPayFileParser.Business
{
    ...
    {
        SettlementDetailDAL settlementDetailDAL = new SettlementDetailDAL();
        ...
        public bool BulkInsertAuth(int clientId, string filePath, StreamWriter objTextWriter, string InputPath, string MoveFilePathWithName, string FileMovePath)
        {
            ...
            try
            {
                ...
                fileImportLogId = fileImportLogManager.InsertDataToFileImportLog(fileName, FileType.SettlementDetail, fileInfo.LastWriteTime);

                DataTable dtSchema = commonDAL.GetTableSchema(AppConstants.Table_SettlementDetail);
                DataTable dtColumnMappings = commonDAL.GetColumnMappingsByFileTypeId((int)FileType.SettlementDetail);

                objTextWriter.WriteLine("Fetching Data From CSV....: {0}----------", DateTime.Now);
                FetchDataInfo objFetchDataInfo = BulkDataInsertManager.FetchDataFromCSV(MoveFilePathWithName, AppConstants.Table_SettlementDetail,
                    Convert.ToString(","), objTextWriter, dtSchema, dtColumnMappings, true);
                objTextWriter.WriteLine("Fetching Data From CSV completed....: {0}----------", DateTime.Now);
                ...
                if (dt.Rows.Count > 0)
                {
                    try
                    {
                        ...
                        BulkDataInsertManager.BulkInsertWithColumnMapping(AppConstants.Table_SettlementDetail, dt, GetColumnMappings(), false);
                        ...
                        settlementDetailDAL.MoveDataToSettlementDetail(fileImportLogId);
                        objTextWriter.WriteLine("Data moved to Main table: {0}----------", DateTime.Now);
                    }
                    catch (Exception ex)
                    {
