...
{
    public class MerchantFileManager
    {
        AdjustmentReservesDAL adjustmentReservesDAL = new AdjustmentReservesDAL();
        ...
        public bool BulkInsertAuth(int clientId, string filePath, StreamWriter objTextWriter, string InputPath, string MoveFilePathWithName, string FileMovePath)
        {
            ...
            {               
                ...
                fileImportLogId = fileImportLogManager.InsertDataToFileImportLog(fileName, FileType.MerchantFile, fileInfo.LastWriteTime);

                DataTable dtSchema = commonDAL.GetTableSchema(AppConstants.Table_MerchantFile);
                DataTable dtColumnMappings = commonDAL.GetColumnMappingsByFileTypeId((int)FileType.MerchantFile);

                FetchDataInfo objFetchDataInfo = BulkDataInsertManager.FetchDataFromCSV(MoveFilePathWithName, AppConstants.Table_MerchantFile,
                ...
                if (dt.Rows.Count > 0)
                {
                    try
                    {
                        ...
                        BulkDataInsertManager.BulkInsertWithColumnMapping(AppConstants.Table_MerchantFile, dt, GetColumnMappings(), false);
                        objTextWriter.WriteLine("Bulk insertion complete : {0}----------", DateTime.Now);

                        objTextWriter.WriteLine("merchant Sync Movement to Main table started.... : {0}----------", DateTime.Now);
                        adjustmentReservesDAL.SyncMerchantFileData(fileImportLogId);
