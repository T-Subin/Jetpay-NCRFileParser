...
{
    public class FundingDepManager
    {
        DepFundingDAL funDepdal = new DepFundingDAL();

        public bool BulkInsertFundingDep(int clientId, string filePath, StreamWriter objTextWriter, string InputPath, string MoveFilePathWithName, string FileMovePath)
        {
            ...
            try
            {
                ...
                try
                {
                    //DataTable dt = Util.ParseCSV(fileName, FileMovePath, AppConstants.Table_FundingDep_JetPay, Convert.ToChar(","), MoveFilePathWithName);

                    DataTable dtSchema = commonDAL.GetTableSchema(AppConstants.Table_FundingDepositSummary);
                    DataTable dtColumnMappings = commonDAL.GetColumnMappingsByFileTypeId((int)FileType.ACHDetail);

                    FetchDataInfo objFetchDataInfo = BulkDataInsertManager.FetchDataFromCSV(MoveFilePathWithName, AppConstants.Table_FundingDep_JetPay,
                        ...
                        foreach (DataRow dr in dt.Rows)
                        {
                            if (!string.IsNullOrEmpty(Convert.ToString(dr["MID"])))
                            {
                                ...
                                if(dr.IsColumnExists("Settle Date"))
                                    dr["Settle Date"] = dr["Settle Date"].ToString().Replace("'", "").NullSafeYYYYMMDDHHMMSSStringToDateTime();
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        //{
                            BulkDataInsertManager.BulkInsertWithColumnMapping(AppConstants.Table_FundingDep_JetPay, dt, GetColumnMappings());
                            objTextWriter.WriteLine("Bulk insertion complete");

                            funDepdal.MoveDataToTransactionTables();
