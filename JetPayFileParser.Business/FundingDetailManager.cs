...
{
    public class FundingDetailManager
    {
        FundingDetailDAL fundingDetailDAL = new FundingDetailDAL();
        ...
        public bool BulkInsertAuth(int clientId, string filePath, StreamWriter objTextWriter, string InputPath, string MoveFilePathWithName, string FileMovePath)
        {
            ...
            try
            {
                ...
                fileImportLogId = fileImportLogManager.InsertDataToFileImportLog(fileName, FileType.FundingDetail, fileInfo.LastWriteTime);

                DataTable dtSchema = commonDAL.GetTableSchema(AppConstants.Table_FundingDetail);
                DataTable dtColumnMappings = commonDAL.GetColumnMappingsByFileTypeId((int)FileType.FundingDetail);

                FetchDataInfo objFetchDataInfo = BulkDataInsertManager.FetchDataFromCSV(MoveFilePathWithName, AppConstants.Table_FundingDetail,
                ...
                if (dt.Rows.Count > 0)
                {
                    try
                    {
                        ...
                        BulkDataInsertManager.BulkInsertWithColumnMapping(AppConstants.Table_FundingDetail, dt, GetColumnMappings(), false);
                        ...
                        fundingDetailDAL.MoveDataToFundingDetail(fileImportLogId);
